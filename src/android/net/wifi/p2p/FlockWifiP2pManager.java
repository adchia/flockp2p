 ...
 * This lets an application discover available peers, setup connection to peers
 * and query for the list of peers. When a p2p connection is formed over wifi,
 * the device continues to maintain the uplink connection over mobile or any
 * other available network for internet connectivity on the device.
 * 
 * <p>
 * The API is asynchronous and responses to requests from an application are on
 * listener callbacks provided by the application. The application needs to do
 * an initialization with {@link #initialize} before doing any p2p operation.
 * 
 * <p>
 * Most application calls need a {@link ActionListener} instance for receiving
 * callbacks {@link ActionListener#onSuccess} or
 * {@link ActionListener#onFailure}. Action callbacks indicate whether the
 * initiation of the action was a success or a failure. Upon failure, the reason
 * of failure can be one of {@link #ERROR}, {@link #P2P_UNSUPPORTED} or
 * {@link #BUSY}.
 * 
 * <p>
 * An application can initiate discovery of peers with {@link #discoverPeers}.
 * An initiated discovery request from an application stays active until the
 * device starts connecting to a peer ,forms a p2p group or there is an explicit
 * {@link #stopPeerDiscovery}. Applications can listen to
 * {@link #WIFI_P2P_DISCOVERY_CHANGED_ACTION} to know if a peer-to-peer
 * discovery is running or stopped. Additionally,
 * {@link #WIFI_P2P_PEERS_CHANGED_ACTION} indicates if the peer list has
 * changed.
 * 
 * <p>
 * When an application needs to fetch the current list of peers, it can request
 * the list of peers with {@link #requestPeers}. When the peer list is available
 ...
 * {@link #connect}. See {@link WifiP2pConfig} for details on setting up the
 * configuration. For communication with legacy Wi-Fi devices that do not
 * support p2p, an app can create a group using {@link #createGroup} which
 * creates an access point whose details can be fetched with
 * {@link #requestGroupInfo}.
 * 
 * <p>
 * After a successful group formation through {@link #createGroup} or through
 * {@link #connect}, use {@link #requestConnectionInfo} to fetch the connection
 * details. The connection info {@link WifiP2pInfo} contains the address of the
 * group owner {@link WifiP2pInfo#groupOwnerAddress} and a flag
 * {@link WifiP2pInfo#isGroupOwner} to indicate if the current device is a p2p
 * group owner. A p2p client can thus communicate with the p2p group owner
 * through a socket connection.
 * 
 * <p>
 * With peer discovery using {@link #discoverPeers}, an application discovers
 * the neighboring peers, but has no good way to figure out which peer to
 * establish a connection with. For example, if a game application is interested
 * in finding all the neighboring peers that are also running the same game, it
 * has no way to find out until after the connection is setup. Pre-association
 * service discovery is meant to address this issue of filtering the peers based
 * on the running services.
 * 
 * <p>
 * With pre-association service discovery, an application can advertise a
 * service for a application on a peer device prior to a connection setup
 * between the devices. Currently, DNS based service discovery (Bonjour) and
 * Upnp are the higher layer protocols supported. Get Bonjour resources at
 * dns-sd.org and Upnp resources at upnp.org As an example, a video application
 * can discover a Upnp capable media renderer prior to setting up a Wi-fi p2p
 * connection with the device.
 * 
 * <p>
 * An application can advertise a Upnp or a Bonjour service with a call to
 * {@link #addLocalService}. After a local service is added, the framework
 * automatically responds to a peer application discovering the service prior to
 * establishing a p2p connection. A call to {@link #removeLocalService} removes
 * a local service and {@link #clearLocalServices} can be used to clear all
 * local services.
 * 
 * <p>
 ...
 * discovery, application can add service discovery request with a call to
 * {@link #addServiceRequest}, remove a service discovery request with a call to
 * {@link #removeServiceRequest} or clear all requests with a call to
 * {@link #clearServiceRequests}. When no service requests remain, a previously
 * running service discovery will stop.
 * 
 * The application is notified of a result of service discovery request through
 * listener callbacks set through {@link #setDnsSdResponseListeners} for Bonjour
 * or {@link #setUpnpServiceResponseListener} for Upnp.
 * 
 * <p class="note">
 * <strong>Note:</strong> Registering an application handler with
 * {@link #initialize} requires the permissions
 * {@link android.Manifest.permission#ACCESS_WIFI_STATE} and
 * {@link android.Manifest.permission#CHANGE_WIFI_STATE} to perform any further
 ...
 * 
 * {@see WifiP2pConfig} {@see WifiP2pInfo} {@see WifiP2pGroup} {@see
 * WifiP2pDevice} {@see WifiP2pDeviceList} {@see android.net.wifi.WpsInfo}
 */
@TargetApi(Build.VERSION_CODES.JELLY_BEAN)
public class FlockWifiP2pManager {
	...
	/**
	 * The lookup key for an int that indicates whether Wi-Fi p2p is enabled or
	 ...
	 * connectivity has changed. One extra {@link #EXTRA_WIFI_P2P_INFO} provides
	 * the p2p connection info in the form of a {@link WifiP2pInfo} object.
	 * Another extra {@link #EXTRA_NETWORK_INFO} provides the network info in
	 * the form of a {@link android.net.NetworkInfo}. A third extra provides the
	 ...
	 * with the p2p network. Retrieve with
	 ...
	 * peers. The full list of peers can also be obtained any time with
	 * {@link #requestPeers}.
	 * 
	...
	/**
	 * The lookup key for a {@link android.net.wifi.p2p.WifiP2pDeviceList}
	 * object representing the new peer list when
	 * {@link #WIFI_P2P_PEERS_CHANGED_ACTION} broadcast is sent.
	 * 
	 * <p>
	 * Retrieve with {@link android.content.Intent#getParcelableExtra(String)}.
	 */
	 ...
	 * or stopped. One extra {@link #EXTRA_DISCOVERY_STATE} indicates whether
	 * discovery has started or stopped.
	 * 
	 * <p>
	 * Note that discovery will be stopped during a connection setup. If the
	 * application tries to re-initiate discovery during this time, it can fail.
	 */
	@SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
	public static final String WIFI_P2P_DISCOVERY_CHANGED_ACTION = "android.net.wifi.p2p.DISCOVERY_STATE_CHANGE";

	/**
	 * The lookup key for an int that indicates whether p2p discovery has
	 * started or stopped. Retrieve it with
	 * {@link android.content.Intent#getIntExtra(String,int)}.
	 * 
	...
	/**
	 * Broadcast intent action indicating that remembered persistent groups have
	 * changed.
	 * 
	 * @hide
	 */
	...
	public FlockWifiP2pManager(WifiP2pManager manager) {
		...
		Field f;
		try {
			f = manager.getClass().getDeclaredField("mService");
			f.setAccessible(true);
			IWifiP2pManager oldService = (IWifiP2pManager) f.get(manager);
			// Construct flock service from normal android service
			mService = new FlockWifiP2pService();
	...
	/**
	 * Passed with {@link ActionListener#onFailure}. Indicates that the
	 * {@link #discoverServices} failed because no service requests are added.
	 * Use {@link #addServiceRequest} to add a service request.
	 */
	...
	public interface ActionListener {
		 ...
		 *            {@link #P2P_UNSUPPORTED}, {@link #ERROR} or {@link #BUSY}
		 */
	...
	public interface GroupInfoListener {
		 ...
		 * 
		 * @param group
		 *            Wi-Fi p2p group info
		 */
		public void onGroupInfoAvailable(WifiP2pGroup group);
	}

	/**
	 * Interface for callback invocation when service discovery response other
	 * than Upnp or Bonjour is received
	 */
	public interface ServiceResponseListener {
		 ...
		 *            service protocol type. The format depends on the service
		 *            type.
		 * @param srcDevice
		 *            source device.
		 */
	...
	/**
	 * Interface for callback invocation when upnp service discovery response is
	 * received
	 * */
	public interface UpnpServiceResponseListener {
		 ...
		 * 
		 * <p>
		 * This function is invoked when the specified device or service is
		 * found.
		 * 
		 * @param uniqueServiceNames
		 *            The list of unique service names.<br>
		 *            e.g)
		 *            uuid:6859dede-8574-59ab-9332-123456789012::urn:schemas
		 *            -upnp-org:device: MediaServer:1
		 * @param srcDevice
		 *            source device.
		 */
	...
	/**
	 * Interface for callback invocation when stored group info list is
	 * available {@hide}
	 */
	public interface PersistentGroupInfoListener {
		 ...
		 * 
		 * @param groups
		 *            Wi-Fi p2p group info list
		 */
		public void onPersistentGroupInfoAvailable(FlockWifiP2pGroupList groups);
	}

	/**
	 * A channel that connects the application to the Wifi p2p framework. Most
	 * p2p operations require a Channel as an argument. An instance of Channel
	 * is obtained by doing a call on {@link #initialize}
	 */
	public static class Channel {
		...
		class P2pHandler extends Handler {
			...
			public void handleMessage(Message message) {
				...
				case RESPONSE_PERSISTENT_GROUP_INFO:
					FlockWifiP2pGroupList groups = (FlockWifiP2pGroupList) message.obj;
					if (listener != null) {
		...
		private void handleUpnpServiceResponse(WifiP2pUpnpServiceResponse resp) {
			mUpnpServRspListener.onUpnpServiceAvailable(
					resp.getUniqueServiceNames(), resp.getSrcDevice());
		}
			...
			if (resp.getDnsType() == WifiP2pDnsSdServiceInfo.DNS_TYPE_PTR) {
				if (mDnsSdServRspListener != null) {
					mDnsSdServRspListener.onDnsSdServiceAvailable(
							resp.getInstanceName(), resp.getDnsQueryName(),
							resp.getSrcDevice());
				}
				...
				if (mDnsSdTxtListener != null) {
					mDnsSdTxtListener.onDnsSdTxtRecordAvailable(
							resp.getDnsQueryName(), resp.getTxtRecord(),
							resp.getSrcDevice());
				}
			} else {
	...
	/**
	 * Registers the application with the Wi-Fi framework. This function must be
	 * the first to be called before any p2p operations are performed.
	 * 
	 * @param srcContext
	 ...
	 * @return Channel instance that is necessary for performing any further p2p
	 *         operations
	 */
	public Channel initialize(Context srcContext, Looper srcLooper,
			FlockWifiP2pManager.ChannelListener listener) {
	...
	/**
	 * Initiate peer discovery. A discovery process involves scanning for
	 ...
	 * to the framework. The application is notified of a success or failure to
	 * initiate discovery through listener callbacks
	 ...
	 * group is formed. Register for {@link #WIFI_P2P_PEERS_CHANGED_ACTION}
	 * intent to determine when the framework notifies of a change as peers are
	 * discovered.
	 * 
	 * <p>
	 * Upon receiving a {@link #WIFI_P2P_PEERS_CHANGED_ACTION} intent, an
	 * application can request for the list of peers using {@link #requestPeers}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param listener
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a stop request to the
	 * framework. The application is notified of a success or failure to
	 * initiate stop through listener callbacks {@link ActionListener#onSuccess}
	 * or {@link ActionListener#onFailure}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param listener
	 ...
	 * to the framework. The application is notified of a success or failure to
	 * initiate connect through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * If the current device is not part of a p2p group, a connect request
	 * initiates a group negotiation with the peer.
	 * 
	 * <p>
	 * If the current device is part of an existing p2p group or has created a
	 * p2p group with {@link #createGroup}, an invitation to join the group is
	 * sent to the peer device.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param config
	 *            options as described in {@link WifiP2pConfig} class
	 * @param listener
	 *            for callbacks on success or failure. Can be null.
	 */
	public void connect(Channel c, WifiP2pConfig config, ActionListener listener) {
		...
		c.mAsyncChannel
				.sendMessage(CONNECT, 0, c.putListener(listener), config);
	}
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a connection
	 * cancellation request to the framework. The application is notified of a
	 * success or failure to initiate cancellation through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 ...
	 * essentially creates an access point that can accept connections from
	 * legacy clients as well as other p2p devices.
	 * 
	 * <p class="note">
	 * <strong>Note:</strong> This function would normally not be used unless
	 * the current device needs to form a p2p connection with a legacy client
	 * 
	 * <p>
	 * The function call immediately returns after sending a group creation
	 * request to the framework. The application is notified of a success or
	 * failure to initiate group creation through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	...
	public void createGroup(Channel c, ActionListener listener) {
		...
		c.mAsyncChannel.sendMessage(CREATE_GROUP,
				WifiP2pGroup.PERSISTENT_NET_ID, c.putListener(listener));
	}
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a group removal
	 * request to the framework. The application is notified of a success or
	 * failure to initiate group removal through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	...
	public void listen(Channel c, boolean enable, ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(enable ? START_LISTEN : STOP_LISTEN, 0,
				c.putListener(listener));
	}

	/** @hide */
	public void setWifiP2pChannels(Channel c, int lc, int oc,
			ActionListener listener) {
		checkChannel(c);
		Bundle p2pChannels = new Bundle();
		p2pChannels.putInt("lc", lc);
		p2pChannels.putInt("oc", oc);
		c.mAsyncChannel.sendMessage(SET_CHANNEL, 0, c.putListener(listener),
				p2pChannels);
	}
	 ...
	 * WPS session. Currently, this is only valid if the current device is
	 * running as a group owner to allow any new clients to join the group. The
	 * application is notified of a success or failure to initiate WPS through
	 * listener callbacks {@link ActionListener#onSuccess} or
	...
	/**
	 * Register a local service for service discovery. If a local service is
	 * registered, the framework automatically responds to a service discovery
	 * request from a peer.
	 * 
	 * <p>
	 * The function call immediately returns after sending a request to add a
	 * local service to the framework. The application is notified of a success
	 * or failure to add service through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	 * The service information is set through {@link WifiP2pServiceInfo}.<br>
	 ...
	 *            for callbacks on success or failure. Can be null.
	 */
	public void addLocalService(Channel c, WifiP2pServiceInfo servInfo,
			ActionListener listener) {
		checkChannel(c);
		checkServiceInfo(servInfo);
		c.mAsyncChannel.sendMessage(ADD_LOCAL_SERVICE, 0,
				c.putListener(listener), servInfo);
	}
	...
	public void removeLocalService(Channel c, WifiP2pServiceInfo servInfo,
		...
		checkChannel(c);
		checkServiceInfo(servInfo);
		c.mAsyncChannel.sendMessage(REMOVE_LOCAL_SERVICE, 0,
				c.putListener(listener), servInfo);
	}
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a request to clear
	 * all local services to the framework. The application is notified of a
	 * success or failure to add service through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	...
	public void clearLocalServices(Channel c, ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(CLEAR_LOCAL_SERVICES, 0,
				c.putListener(listener));
	}

	/**
	 * Register a callback to be invoked on receiving service discovery
	 * response. Used only for vendor specific protocol right now. For Bonjour
	 * or Upnp, use {@link #setDnsSdResponseListeners} or
	 * {@link #setUpnpServiceResponseListener} respectively.
	 * 
	 * <p>
	 ...
	 * service discovery to the framework. The application is notified of a
	 * success or failure to initiate discovery through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * The application is notified of the response against the service discovery
	 * request through listener callbacks registered by
	 * {@link #setServiceResponseListener} or {@link #setDnsSdResponseListeners}
	 * , or {@link #setUpnpServiceResponseListener}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param listener
	...
	public void discoverServices(Channel c, ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(DISCOVER_SERVICES, 0,
				c.putListener(listener));
	}
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a request to add
	 * service discovery request to the framework. The application is notified
	 * of a success or failure to add service through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	 * After service discovery request is added, you can initiate service
	 * discovery by {@link #discoverServices}.
	 * 
	 * <p>
	 * The added service requests can be cleared with calls to
	 * {@link #removeServiceRequest(Channel, WifiP2pServiceRequest, ActionListener)}
	 * or {@link #clearServiceRequests(Channel, ActionListener)}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param req
	 ...
	 *            for callbacks on success or failure. Can be null.
	 */
	public void addServiceRequest(Channel c, WifiP2pServiceRequest req,
			ActionListener listener) {
		checkChannel(c);
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a request to remove
	 * service discovery request to the framework. The application is notified
	 * of a success or failure to add service through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 ...
	 * <p>
	 * The function call immediately returns after sending a request to clear
	 * all service discovery requests to the framework. The application is
	 * notified of a success or failure to add service through listener
	 * callbacks {@link ActionListener#onSuccess} or
	 * {@link ActionListener#onFailure}.
	 * 
	 * @param c
	...
	public void clearServiceRequests(Channel c, ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(CLEAR_SERVICE_REQUESTS, 0,
				c.putListener(listener));
	}
	...
	public void requestConnectionInfo(Channel c, ConnectionInfoListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(REQUEST_CONNECTION_INFO, 0,
				c.putListener(listener));
	}
	...
	public void requestGroupInfo(Channel c, GroupInfoListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(REQUEST_GROUP_INFO, 0,
				c.putListener(listener));
	}
	...
	public void setDeviceName(Channel c, String devName, ActionListener listener) {
		...
		c.mAsyncChannel.sendMessage(SET_DEVICE_NAME, 0,
				c.putListener(listener), d);
	}

	/** @hide */
	public void setWFDInfo(Channel c, WifiP2pWfdInfo wfdInfo,
			ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(SET_WFD_INFO, 0, c.putListener(listener),
				wfdInfo);
	}
	 ...
	 * 
	 * <p>
	 * The function call immediately returns after sending a persistent group
	 * removal request to the framework. The application is notified of a
	 * success or failure to initiate group removal through listener callbacks
	 * {@link ActionListener#onSuccess} or {@link ActionListener#onFailure}.
	 * 
	 * <p>
	 * The persistent p2p group list stored in the system can be obtained by
	 * {@link #requestPersistentGroupInfo(Channel, PersistentGroupInfoListener)}
	 * and a network id can be obtained by {@link WifiP2pGroup#getNetworkId()}.
	 * 
	 * @param c
	 *            is the channel created at {@link #initialize}
	 * @param netId
	 ...
	 * @hide
	 */
	public void deletePersistentGroup(Channel c, int netId,
			ActionListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(DELETE_PERSISTENT_GROUP, netId,
				c.putListener(listener));
	}
	 ...
	 *            is the channel created at {@link #initialize}
	 * @param listener
	 *            for callback when persistent group info list is available. Can
	 *            be null.
	 * @hide
	 */
	public void requestPersistentGroupInfo(Channel c,
			PersistentGroupInfoListener listener) {
		checkChannel(c);
		c.mAsyncChannel.sendMessage(REQUEST_PERSISTENT_GROUP_INFO, 0,
				c.putListener(listener));
	}
	...
	/**
	 * Get a reference to WifiP2pService handler. This is used to establish an
	 * AsyncChannel communication with WifiService
	 * 
