...
import android.net.wifi.p2p.FlockWifiP2pGroupList.GroupDeleteListener;
 ...
 * Applications communicate with this service to issue device discovery and
 * connectivity requests through the WifiP2pManager interface. The state machine
 * communicates with the wifi driver through wpa_supplicant and handles the
 * event responses through WifiMonitor.
 * 
 * Note that the term Wifi when used without a p2p suffix refers to the client
 * mode of Wifi operation
 * 
 * @hide
 */
@TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
public class FlockWifiP2pService extends IWifiP2pManager.Stub {
	...
	/*
	 * These commands are used to temporarily disconnect wifi when we detect a
	 * frequency conflict which would make it impossible to have with p2p and
	 * wifi active at the same time.
	 * 
	 * If the user chooses to disable wifi temporarily, we keep wifi
	 * disconnected until the p2p connection is done and terminated at which
	 * point we will bring back wifi up
	 * 
	 * DISCONNECT_WIFI_REQUEST msg.arg1 = 1 enables temporary disconnect and 0
	 * disables it.
	 */
	 ...
	 * even after all clients have been disconnected until an explicit remove is
	 * invoked
	 */
	...
	/*
	 * Track whether we are in p2p discovery. This is used to avoid sending
	 * duplicate broadcasts
	 */
	private boolean mDiscoveryStarted;
	/*
	 * Track whether servcice/peer discovery is blocked in favor of other wifi
	 * actions (notably dhcp)
	 */
	private boolean mDiscoveryBlocked;

	// Supplicant doesn't like setting the same country code multiple times (it
	// may drop
	// current connected network), so we save the country code here to avoid
	// redundency
	...
	/*
	 * Service discovery request ID of wpa_supplicant. null means it's not set
	 * yet.
	 */
	...
	/*
	 * Is chosen as a unique range to avoid conflict with the range defined in
	 * Tethering.java
	 */
	...
	/**
	 * Error code definition. see the Table.8 in the WiFi Direct specification
	 * for the detail.
	 */
	public static enum P2pStatus {
		...
		/*
		 * The target device reached the limit of the number of the connectable
		 * device. For example, device limit or group limit is set.
		 */
		LIMIT_REACHED,
		...
		/*
		 * Unknown p2p group. For example, Device A tries to invoke the previous
		 * persistent group, but device B has removed the specified credential
		 * already.
		 */
		UNKNOWN_P2P_GROUP,

		/*
		 * Both p2p devices indicated an intent of 15 in group owner
		 * negotiation.
		 */
		BOTH_GO_INTENT_15,
	...
	public FlockWifiP2pService(Context context) {
		mContext = context;

		// STOPSHIP: get this from native side
		mInterface = "p2p0";
		mNetworkInfo = new NetworkInfo(ConnectivityManager.TYPE_WIFI_P2P, 0,
				NETWORKTYPE, "");
	...
	public FlockWifiP2pService() {
		...
		this.mNwService = (INetworkManagementService) getField("mNwService",
				oldService);
		this.mDhcpStateMachine = (DhcpStateMachine) getField(
				"mDhcpStateMachine", oldService);
		this.mP2pStateMachine = (P2pStateMachine) getField("mP2pStateMachine",
				oldService);
		this.mReplyChannel = (AsyncChannel) getField("mReplyChannel",
				oldService);
		this.mWifiChannel = (AsyncChannel) getField("mWifiChannel", oldService);
		this.mP2pSupported = (Boolean) getField("mP2pSupported", oldService);
		this.mThisDevice = (WifiP2pDevice) getField("mThisDevice", oldService);
		this.mAutonomousGroup = (Boolean) getField("mAutonomousGroup",
				oldService);
		this.mAutonomousGroup = (Boolean) getField("mAutonomousGroup",
				oldService);
		this.mDiscoveryStarted = (Boolean) getField("mDiscoveryStarted",
				oldService);
		this.mDiscoveryBlocked = (Boolean) getField("mDiscoveryBlocked",
				oldService);
		this.mDiscoveryPostponed = (Boolean) getField("mDiscoveryPostponed",
				oldService);
		this.mNetworkInfo = (NetworkInfo) getField("mNetworkInfo", oldService);
		this.mTempoarilyDisconnectedWifi = (Boolean) getField(
				"mTempoarilyDisconnectedWifi", oldService);
		this.mServiceTransactionId = (Byte) getField("mServiceTransactionId",
				oldService);
		this.mServiceDiscReqId = (String) getField("mServiceDiscReqId",
				oldService);
		this.mClientInfoList = (HashMap<Messenger, ClientInfo>) getField(
				"mClientInfoList", oldService);
	}

	public Object getField(String fieldName, Object o) {
		try {
			Log.d("class of" + fieldName, ": " + o.getClass().toString());
	...
	public void connectivityServiceReady() {
		IBinder b = ServiceManager.getService(NETWORKMANAGEMENT_SERVICE);
		mNwService = INetworkManagementService.Stub.asInterface(b);
	}

	private void enforceAccessPermission() {
		mContext.enforceCallingOrSelfPermission(
				android.Manifest.permission.ACCESS_WIFI_STATE, "WifiP2pService");
	}

	private void enforceChangePermission() {
		mContext.enforceCallingOrSelfPermission(
				android.Manifest.permission.CHANGE_WIFI_STATE, "WifiP2pService");
	}
	...
	/**
	 * Get a reference to handler. This is used by a client to establish an
	 * AsyncChannel communication with WifiP2pService
	 */
	...
	/**
	 * This is used to provide information to drivers to optimize performance
	 * depending on the current mode of operation. 0 - disabled 1 - source
	 * operation 2 - sink operation
	 * 
	 * As an example, the driver could reduce the channel dwell time during
	 * scanning when acting as a source or sink to minimize impact on miracast.
	 */
	...
	private class P2pStateMachine extends StateMachine {
		...
		/*
		 * During a connection, supplicant can tell us that a device was lost.
		 * From a supplicant's perspective, the discovery stops during
		 * connection and it purges device since it does not get latest updates
		 * about the device without being in discovery state.
		 * 
		 * From the framework perspective, the device is still there since we
		 * are connecting or connected to it. so we keep these devices in a
		 * separate list, so that they are removed when connection is cancelled
		 * or lost
		 */
		private final WifiP2pDeviceList mPeersLostDuringConnection = new WifiP2pDeviceList();
		private final FlockWifiP2pGroupList mGroups = new FlockWifiP2pGroupList(
				null, new GroupDeleteListener() {
					@Override
		...
		// Saved WifiP2pConfig for an ongoing peer connection. This will never
		// be null.
		...
		P2pStateMachine(String name, boolean p2pSupported) {
			...
			addState(mUserAuthorizingNegotiationRequestState,
					mGroupCreatingState);
			...
			public boolean processMessage(Message message) {
				...
				case AsyncChannel.CMD_CHANNEL_DISCONNECTED:
					if (message.arg1 == AsyncChannel.STATUS_SEND_UNSUCCESSFUL) {
						...
						loge("Client connection lost with reason: "
								+ message.arg1);
					}
					mWifiChannel = null;
					break;
				...
				case BLOCK_DISCOVERY:
					...
					// always reset this - we went to a state that doesn't
					// support discovery so
					...
					if (mDiscoveryBlocked) {
						try {
							...
							loge("unable to send BLOCK_DISCOVERY response: "
									+ e);
						}
					}
					break;
				case FlockWifiP2pManager.DISCOVER_PEERS:
					replyToMessage(message,
							FlockWifiP2pManager.DISCOVER_PEERS_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.STOP_DISCOVERY:
					replyToMessage(message,
							FlockWifiP2pManager.STOP_DISCOVERY_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.DISCOVER_SERVICES:
					replyToMessage(message,
							FlockWifiP2pManager.DISCOVER_SERVICES_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.CONNECT:
					replyToMessage(message, FlockWifiP2pManager.CONNECT_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.CANCEL_CONNECT:
					replyToMessage(message,
							FlockWifiP2pManager.CANCEL_CONNECT_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.CREATE_GROUP:
					replyToMessage(message,
							FlockWifiP2pManager.CREATE_GROUP_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.REMOVE_GROUP:
					replyToMessage(message,
							FlockWifiP2pManager.REMOVE_GROUP_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.ADD_LOCAL_SERVICE:
					replyToMessage(message,
							FlockWifiP2pManager.ADD_LOCAL_SERVICE_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.REMOVE_LOCAL_SERVICE:
					replyToMessage(message,
							FlockWifiP2pManager.REMOVE_LOCAL_SERVICE_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.CLEAR_LOCAL_SERVICES:
					replyToMessage(message,
							FlockWifiP2pManager.CLEAR_LOCAL_SERVICES_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.ADD_SERVICE_REQUEST:
					replyToMessage(message,
							FlockWifiP2pManager.ADD_SERVICE_REQUEST_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.REMOVE_SERVICE_REQUEST:
					replyToMessage(message,
							FlockWifiP2pManager.REMOVE_SERVICE_REQUEST_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.CLEAR_SERVICE_REQUESTS:
					replyToMessage(message,
							FlockWifiP2pManager.CLEAR_SERVICE_REQUESTS_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.SET_DEVICE_NAME:
					replyToMessage(message,
							FlockWifiP2pManager.SET_DEVICE_NAME_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.DELETE_PERSISTENT_GROUP:
					replyToMessage(message,
							FlockWifiP2pManager.DELETE_PERSISTENT_GROUP,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.SET_WFD_INFO:
					replyToMessage(message,
							FlockWifiP2pManager.SET_WFD_INFO_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				case FlockWifiP2pManager.REQUEST_PEERS:
					replyToMessage(message, FlockWifiP2pManager.RESPONSE_PEERS,
							new WifiP2pDeviceList(mPeers));
					break;
				case FlockWifiP2pManager.REQUEST_CONNECTION_INFO:
					replyToMessage(message,
							FlockWifiP2pManager.RESPONSE_CONNECTION_INFO,
							new WifiP2pInfo(mWifiP2pInfo));
					break;
				case FlockWifiP2pManager.REQUEST_GROUP_INFO:
					replyToMessage(message,
							FlockWifiP2pManager.RESPONSE_GROUP_INFO,
							mGroup != null ? new WifiP2pGroup(mGroup) : null);
					break;
				case FlockWifiP2pManager.REQUEST_PERSISTENT_GROUP_INFO:
					replyToMessage(message,
							FlockWifiP2pManager.RESPONSE_PERSISTENT_GROUP_INFO,
							new FlockWifiP2pGroupList(mGroups, null));
					break;
				case FlockWifiP2pManager.START_WPS:
					replyToMessage(message,
							FlockWifiP2pManager.START_WPS_FAILED,
							FlockWifiP2pManager.BUSY);
					break;
				// Ignore
				...
				case FlockWifiP2pManager.START_LISTEN:
				case FlockWifiP2pManager.STOP_LISTEN:
				case FlockWifiP2pManager.SET_CHANNEL:
				...
				case WifiStateMachine.CMD_DISABLE_P2P_REQ:
					...
					mWifiChannel
							.sendMessage(WifiStateMachine.CMD_DISABLE_P2P_RSP);
					break;
		...
		class P2pNotSupportedState extends State {
			...
			public boolean processMessage(Message message) {
				...
				case FlockWifiP2pManager.DISCOVER_PEERS:
					replyToMessage(message,
							FlockWifiP2pManager.DISCOVER_PEERS_FAILED,
							FlockWifiP2pManager.P2P_UNSUPPORTED);
					break;
				case FlockWifiP2pManager.STOP_DISCOVERY:
					replyToMessage(message,
							FlockWifiP2pManager.STOP_DISCOVERY_FAILED,
				...
				case FlockWifiP2pManager.START_LISTEN:
					replyToMessage(message,
							FlockWifiP2pManager.START_LISTEN_FAILED,
							FlockWifiP2pManager.P2P_UNSUPPORTED);
					break;
				case FlockWifiP2pManager.STOP_LISTEN:
					replyToMessage(message,
							FlockWifiP2pManager.STOP_LISTEN_FAILED,
							FlockWifiP2pManager.P2P_UNSUPPORTED);
					break;
		...
		class P2pDisablingState extends State {
			...
			public void enter() {
				if (DBG)
					logd(getName());
				sendMessageDelayed(
						obtainMessage(DISABLE_P2P_TIMED_OUT,
			...
			public boolean processMessage(Message message) {
				...
				case WifiMonitor.SUP_DISCONNECTION_EVENT:
					if (DBG)
						logd("p2p socket connection lost");
		...
		class P2pEnablingState extends State {
			...
			public boolean processMessage(Message message) {
				...
				case WifiMonitor.SUP_CONNECTION_EVENT:
					if (DBG)
						logd("P2p socket connection successful");
			...
			public boolean processMessage(Message message) {
				...
				case FlockWifiP2pManager.SET_DEVICE_NAME: {
					...
					if (d != null && setAndPersistDeviceName(d.deviceName)) {
						...
						replyToMessage(message,
								FlockWifiP2pManager.SET_DEVICE_NAME_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.SET_DEVICE_NAME_FAILED,
				...
				case FlockWifiP2pManager.SET_WFD_INFO: {
					...
					if (d != null && setWfdInfo(d)) {
						replyToMessage(message,
								FlockWifiP2pManager.SET_WFD_INFO_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.SET_WFD_INFO_FAILED,
								FlockWifiP2pManager.ERROR);
				...
				case FlockWifiP2pManager.STOP_DISCOVERY:
					if (mWifiNative.p2pStopFind()) {
						replyToMessage(message,
								FlockWifiP2pManager.STOP_DISCOVERY_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.STOP_DISCOVERY_FAILED,
								FlockWifiP2pManager.ERROR);
				...
				case FlockWifiP2pManager.DISCOVER_SERVICES:
					...
					if (!updateSupplicantServiceRequest()) {
						replyToMessage(message,
								FlockWifiP2pManager.DISCOVER_SERVICES_FAILED,
								FlockWifiP2pManager.NO_SERVICE_REQUESTS);
						break;
					}
					if (mWifiNative.p2pFind(DISCOVER_TIMEOUT_S)) {
						replyToMessage(message,
								FlockWifiP2pManager.DISCOVER_SERVICES_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.DISCOVER_SERVICES_FAILED,
								FlockWifiP2pManager.ERROR);
				...
				case FlockWifiP2pManager.ADD_LOCAL_SERVICE:
					...
					if (addLocalService(message.replyTo, servInfo)) {
						replyToMessage(message,
								FlockWifiP2pManager.ADD_LOCAL_SERVICE_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.ADD_LOCAL_SERVICE_FAILED);
					}
				...
				case FlockWifiP2pManager.REMOVE_LOCAL_SERVICE:
					...
					replyToMessage(message,
							FlockWifiP2pManager.REMOVE_LOCAL_SERVICE_SUCCEEDED);
					break;
				case FlockWifiP2pManager.CLEAR_LOCAL_SERVICES:
					if (DBG)
					...
					replyToMessage(message,
							FlockWifiP2pManager.CLEAR_LOCAL_SERVICES_SUCCEEDED);
					break;
				case FlockWifiP2pManager.ADD_SERVICE_REQUEST:
					if (DBG)
					...
					replyToMessage(message,
							FlockWifiP2pManager.ADD_SERVICE_REQUEST_SUCCEEDED);
					break;
				case FlockWifiP2pManager.REMOVE_SERVICE_REQUEST:
					if (DBG)
						logd(getName() + " remove service request");
					removeServiceRequest(message.replyTo,
							(WifiP2pServiceRequest) message.obj);
					replyToMessage(
							message,
							FlockWifiP2pManager.REMOVE_SERVICE_REQUEST_SUCCEEDED);
					break;
				case FlockWifiP2pManager.CLEAR_SERVICE_REQUESTS:
					if (DBG)
					...
					replyToMessage(
							message,
							FlockWifiP2pManager.CLEAR_SERVICE_REQUESTS_SUCCEEDED);
					break;
				case WifiMonitor.P2P_SERV_DISC_RESP_EVENT:
					if (DBG)
				...
				case FlockWifiP2pManager.DELETE_PERSISTENT_GROUP:
					...
					replyToMessage(
							message,
							FlockWifiP2pManager.DELETE_PERSISTENT_GROUP_SUCCEEDED);
					break;
				case SET_MIRACAST_MODE:
				...
				case FlockWifiP2pManager.START_LISTEN:
					...
					if (mWifiNative.p2pExtListen(true, 500, 500)) {
						replyToMessage(message,
								FlockWifiP2pManager.START_LISTEN_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.START_LISTEN_FAILED);
					}
				...
				case FlockWifiP2pManager.STOP_LISTEN:
					...
					if (mWifiNative.p2pExtListen(false, 0, 0)) {
						replyToMessage(message,
								FlockWifiP2pManager.STOP_LISTEN_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.STOP_LISTEN_FAILED);
					}
				...
				case FlockWifiP2pManager.SET_CHANNEL:
					...
					if (mWifiNative.p2pSetChannel(lc, oc)) {
						replyToMessage(message,
								FlockWifiP2pManager.SET_CHANNEL_SUCCEEDED);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.SET_CHANNEL_FAILED);
					}
					break;
				case SET_COUNTRY_CODE:
			...
			public boolean processMessage(Message message) {
				...
				case FlockWifiP2pManager.CONNECT:
					...
					if (isConfigInvalid(config)) {
						...
						replyToMessage(message,
								FlockWifiP2pManager.CONNECT_FAILED);
						break;
					}
					...
					sendPeersChangedBroadcast();
					replyToMessage(message,
							FlockWifiP2pManager.CONNECT_SUCCEEDED);
					break;
				case FlockWifiP2pManager.STOP_DISCOVERY:
					if (mWifiNative.p2pStopFind()) {
						// When discovery stops in inactive state, flush to
						// clear
						// state peer data
						mWifiNative.p2pFlush();
				...
				case WifiMonitor.P2P_INVITATION_RECEIVED_EVENT:
					...
					// Check if we have the owner in peer list and use
					// appropriate
				...
				case WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT:
					// We let the supplicant handle the provision discovery
					// response
					// and wait instead for the GO_NEGOTIATION_REQUEST_EVENT.
					// Handling provision discovery and issuing a p2p_connect
					// before
				...
				case FlockWifiP2pManager.CREATE_GROUP:
					...
					if (ret) {
						replyToMessage(message,
								FlockWifiP2pManager.CREATE_GROUP_SUCCEEDED);
						transitionTo(mGroupNegotiationState);
					} else {
						replyToMessage(message,
								FlockWifiP2pManager.CREATE_GROUP_FAILED,
		...
		class GroupCreatingState extends State {
			...
			public void enter() {
				if (DBG)
					logd(getName());
				sendMessageDelayed(
						obtainMessage(GROUP_CREATING_TIMED_OUT,
			...
			public boolean processMessage(Message message) {
				...
				case GROUP_CREATING_TIMED_OUT:
					if (mGroupCreatingTimeoutIndex == message.arg1) {
						if (DBG)
							logd("Group negotiation timed out");
				...
				case FlockWifiP2pManager.CANCEL_CONNECT:
					...
					// discovery or for a pending user action, but at the
					// framework
					...
					transitionTo(mInactiveState);
					replyToMessage(message,
							FlockWifiP2pManager.CANCEL_CONNECT_SUCCEEDED);
					break;
				default:
					ret = NOT_HANDLED;
				}
			...
			public boolean processMessage(Message message) {
				...
				case WifiMonitor.P2P_PROV_DISC_PBC_RSP_EVENT:
					...
					if (!device.deviceAddress
							.equals(mSavedPeerConfig.deviceAddress))
						break;
		...
		class GroupNegotiationState extends State {
			...
			public boolean processMessage(Message message) {
				if (DBG)
					logd(getName() + message.toString());
				switch (message.what) {
				// We ignore these right now, since we get a GROUP_STARTED
				// notification
				// afterwards
				...
				case WifiMonitor.P2P_GROUP_STARTED_EVENT:
					...
					if (mGroup.getNetworkId() == WifiP2pGroup.PERSISTENT_NET_ID) {
						/*
						 * update cache information and set network id to
						 * mGroup.
						 */
					...
					if (mGroup.isGroupOwner()) {
						/*
						 * Setting an idle time out on GO causes issues with
						 * certain scenarios on clients where it can be
						 * off-channel for longer and with the power save modes
						 * used.
						 * 
						 * TODO: Verify multi-channel scenarios and supplicant
						 * behavior are better before adding a time out in
						 * future
						 */
						// Set group idle timeout of 10 sec, to avoid GO
						// beaconing incase of any
						...
						mWifiNative.setP2pGroupIdle(mGroup.getInterface(),
								GROUP_IDLE_TIME_S);
						mDhcpStateMachine = DhcpStateMachine
								.makeDhcpStateMachine(mContext,
										P2pStateMachine.this,
										mGroup.getInterface());
						// TODO: We should use DHCP state machine PRE message
						// like WifiStateMachine
						mWifiNative.setP2pPowerSave(mGroup.getInterface(),
								false);
						mDhcpStateMachine
								.sendMessage(DhcpStateMachine.CMD_START_DHCP);
						WifiP2pDevice groupOwner = mGroup.getOwner();
						WifiP2pDevice peer = mPeers
								.get(groupOwner.deviceAddress);
						if (peer != null) {
							// update group owner details with peer details
							// found at discovery
							...
							// group owner address (all zeroes) at times. Avoid
							// a
							// crash, but continue group creation since it is
							// not
							// essential.
				...
				case WifiMonitor.P2P_INVITATION_RESULT_EVENT:
					...
					if (status == P2pStatus.UNKNOWN_P2P_GROUP) {
						...
						if (netId >= 0) {
							...
							removeClientFromList(netId,
									mSavedPeerConfig.deviceAddress, true);
						}
						...
						// Devices setting persistent_reconnect to 0 in
						// wpa_supplicant
		...
		class FrequencyConflictState extends State {
			...
			private void notifyFrequencyConflict() {
				...
				AlertDialog dialog = new AlertDialog.Builder(mContext)
						.setMessage(
								r.getString(
										R.string.wifi_p2p_frequency_conflict_message,
										getDeviceName(mSavedPeerConfig.deviceAddress)))
						.setPositiveButton(r.getString(R.string.dlg_ok),
								new OnClickListener() {
									@Override
			...
			public boolean processMessage(Message message) {
				...
				case WifiMonitor.P2P_GROUP_STARTED_EVENT:
					loge(getName()
							+ "group started after freq conflict, handle anyway");
					deferMessage(message);
					transitionTo(mGroupNegotiationState);
					break;
				case WifiMonitor.P2P_GO_NEGOTIATION_FAILURE_EVENT:
				...
				case DROP_WIFI_USER_ACCEPT:
					...
					mWifiChannel.sendMessage(
							FlockWifiP2pService.DISCONNECT_WIFI_REQUEST, 1);
				...
				case DISCONNECT_WIFI_RESPONSE:
					...
					transitionTo(mInactiveState);
					sendMessage(FlockWifiP2pManager.CONNECT, mSavedPeerConfig);
					break;
				default:
					return NOT_HANDLED;
				}
		...
		class GroupCreatedState extends State {
			...
			public void enter() {
				...
				mNetworkInfo.setDetailedState(
						NetworkInfo.DetailedState.CONNECTED, null, null);

				updateThisDevice(WifiP2pDevice.CONNECTED);

				// DHCP server has already been started if I am a group owner
				if (mGroup.isGroupOwner()) {
					setWifiP2pInfoOnGroupFormation(NetworkUtils
							.numericToInetAddress(SERVER_ADDRESS));
				}
			...
			public boolean processMessage(Message message) {
					...
					if (deviceAddress != null) {
						...
						if (mGroup.removeClient(deviceAddress)) {
							...
							if (!mAutonomousGroup && mGroup.isClientListEmpty()) {
								...
								mWifiNative.p2pGroupRemove(mGroup
										.getInterface());
								// We end up sending connection changed
								// broadcast
				...
				case DhcpStateMachine.CMD_POST_DHCP_ACTION:
					...
					if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS
						...
						mWifiNative
								.setP2pPowerSave(mGroup.getInterface(), true);
					} else {
						loge("DHCP failed");
						mWifiNative.p2pGroupRemove(mGroup.getInterface());
					}
				...
				case FlockWifiP2pManager.REMOVE_GROUP:
					...
					if (mWifiNative.p2pGroupRemove(mGroup.getInterface())) {
						...
						replyToMessage(message,
								FlockWifiP2pManager.REMOVE_GROUP_SUCCEEDED);
					} else {
						handleGroupRemoved();
						transitionTo(mInactiveState);
						replyToMessage(message,
					...
					break;
				/*
				 * We do not listen to NETWORK_DISCONNECTION_EVENT for group
				 * removal handling since supplicant actually tries to reconnect
				 * after a temporary disconnect until group idle time out.
				 * Eventually, a group removal event will come when group has
				 * been removed.
				 * 
				 * When there are connectivity issues during temporary
				 * disconnect, the application will also just remove the group.
				 * 
				 * Treating network disconnection as group removal causes race
				 * conditions since supplicant would still maintain the group at
				 * that stage.
				 */
				case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
					if (DBG)
				...
				case WifiMonitor.P2P_DEVICE_LOST_EVENT:
					device = (WifiP2pDevice) message.obj;
					// Device loss for a connected device indicates it is not in
					// discovery any more
					...
					return NOT_HANDLED;
				case WifiStateMachine.CMD_DISABLE_P2P_REQ:
					sendMessage(FlockWifiP2pManager.REMOVE_GROUP);
					deferMessage(message);
					break;
				...
				case FlockWifiP2pManager.START_WPS:
					...
					if (wps.setup == WpsInfo.PBC) {
						ret = mWifiNative.startWpsPbc(mGroup.getInterface(),
								null);
					} else {
						if (wps.pin == null) {
							String pin = mWifiNative.startWpsPinDisplay(mGroup
									.getInterface());
							try {
							...
							ret = mWifiNative.startWpsPinKeypad(
									mGroup.getInterface(), wps.pin);
						}
					}
					replyToMessage(message,
							ret ? FlockWifiP2pManager.START_WPS_SUCCEEDED
									: FlockWifiP2pManager.START_WPS_FAILED);
					break;
				case FlockWifiP2pManager.CONNECT:
					WifiP2pConfig config = (WifiP2pConfig) message.obj;
					...
					if (mWifiNative.p2pInvite(mGroup, config.deviceAddress)) {
						mPeers.updateStatus(config.deviceAddress,
								WifiP2pDevice.INVITED);
						sendPeersChangedBroadcast();
						replyToMessage(message,
								FlockWifiP2pManager.CONNECT_SUCCEEDED);
					...
					// TODO: figure out updating the status to declined when
				...
				case WifiMonitor.P2P_INVITATION_RESULT_EVENT:
					...
					if (status == P2pStatus.UNKNOWN_P2P_GROUP) {
						...
						if (netId >= 0) {
							...
							// try invitation.
							sendMessage(FlockWifiP2pManager.CONNECT,
									mSavedPeerConfig);
						}
					}
					break;
			...
			public void exit() {
				...
				resetWifiP2pInfo();
				mNetworkInfo.setDetailedState(
						NetworkInfo.DetailedState.DISCONNECTED, null, null);
				sendP2pConnectionChangedBroadcast();
			}
		}
		...
		class UserAuthorizingJoinState extends State {
			...
			public boolean processMessage(Message message) {
				...
				case PEER_CONNECTION_USER_REJECT:
					if (DBG)
						logd("User rejected incoming request");
					transitionTo(mGroupCreatedState);
					break;
				default:
					return NOT_HANDLED;
		...
		private void sendP2pDiscoveryChangedBroadcast(boolean started) {
			...
			final Intent intent = new Intent(
					FlockWifiP2pManager.WIFI_P2P_DISCOVERY_CHANGED_ACTION);
			intent.addFlags(FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
		...
		private void sendPeersChangedBroadcast() {
			final Intent intent = new Intent(
					FlockWifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION);
		...
		private void sendP2pConnectionChangedBroadcast() {
			...
			Intent intent = new Intent(
					FlockWifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION);
			intent.addFlags(FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT
					| Intent.FLAG_RECEIVER_REPLACE_PENDING);
			intent.putExtra(FlockWifiP2pManager.EXTRA_WIFI_P2P_INFO,
					new WifiP2pInfo(mWifiP2pInfo));
			intent.putExtra(FlockWifiP2pManager.EXTRA_NETWORK_INFO,
		...
		private void startDhcpServer(String intf) {
			...
			try {
				...
				ifcg.setLinkAddress(new LinkAddress(NetworkUtils
						.numericToInetAddress(SERVER_ADDRESS), 24));
				ifcg.setInterfaceUp();
		...
		private void notifyP2pEnableFailure() {
			...
			AlertDialog dialog = new AlertDialog.Builder(mContext)
					...
					.setPositiveButton(r.getString(R.string.ok), null).create();
			dialog.getWindow().setType(
					WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
			dialog.show();
		}
		...
		private void addRowToDialog(ViewGroup group, int stringId, String value) {
			...
			View row = LayoutInflater.from(mContext).inflate(
					R.layout.wifi_p2p_dialog_row, group, false);
			((TextView) row.findViewById(R.id.name)).setText(r
					.getString(stringId));
		...
		private void notifyInvitationSent(String pin, String peerAddress) {
			...
			final View textEntryView = LayoutInflater.from(mContext).inflate(
					R.layout.wifi_p2p_dialog, null);

			ViewGroup group = (ViewGroup) textEntryView.findViewById(R.id.info);
			addRowToDialog(group, R.string.wifi_p2p_to_message,
					getDeviceName(peerAddress));
			addRowToDialog(group, R.string.wifi_p2p_show_pin_message, pin);

			AlertDialog dialog = new AlertDialog.Builder(mContext)
					.setTitle(
							r.getString(R.string.wifi_p2p_invitation_sent_title))
		...
		private void notifyInvitationReceived() {
			...
			ViewGroup group = (ViewGroup) textEntryView.findViewById(R.id.info);
			addRowToDialog(group, R.string.wifi_p2p_from_message,
					getDeviceName(mSavedPeerConfig.deviceAddress));

			final EditText pin = (EditText) textEntryView
					.findViewById(R.id.wifi_p2p_wps_pin);

			sendMessage(PEER_CONNECTION_USER_ACCEPT);
		}

		/**
		 * Synchronize the persistent group list between wpa_supplicant and
		 * mGroups.
		 */
		private void updatePersistentNetworks(boolean reload) {
			...
			for (int i = 1; i < lines.length; i++) {
				...
				if (flags.indexOf("[P2P-PERSISTENT]") == -1) {
					/*
					 * The unused profile is sometimes remained when the p2p
					 * group formation is failed. So, we clean up the p2p group
					 * here.
					 */
					if (DBG)
						logd("clean up the unused persistent group. netId="
								+ netId);
					mWifiNative.removeNetwork(netId);
					isSaveRequired = true;
					continue;
		...
		/*
		 * TODO: The supplicant does not provide group capability changes as an
		 * event. Having it pushed as an event would avoid polling for this
		 * information right before a connection
		 */
		...
		private boolean reinvokePersistentGroup(WifiP2pConfig config) {
			...
			if (!join && dev.isInvitationCapable()) {
				...
				if (config.netId >= 0) {
					if (config.deviceAddress.equals(mGroups
							.getOwnerAddr(config.netId))) {
						netId = config.netId;
					}
				} else {
				...
				if (DBG)
					logd("netId related with " + dev.deviceAddress + " = "
							+ netId);
				if (netId >= 0) {
		...
		/**
		 * Return the network id of the group owner profile which has the p2p
		 * client with the specified device address in it's client list. If more
		 * than one persistent group of the same address is present in its
		 * client lists, return the first one.
		 * 
		 * @param deviceAddress
		 *            p2p device address.
		...
		private String[] getClientList(int netId) {
			String p2pClients = mWifiNative.getNetworkVariable(netId,
					"p2p_client_list");
			if (p2pClients == null) {
				return null;
			}
		 ...
		 * 
		 * @param netId
		 *            network id of the profile.
		 * @param addr
		 *            p2p client address to be removed.
		 * @param isRemovable
		 *            if true, remove the specified profile if its client list
		 *            becomes empty.
		 * @return whether removing the specified p2p client is successful or
		 *         not.
		 */
		private boolean removeClientFromList(int netId, String addr,
				boolean isRemovable) {
			...
			mWifiNative.setNetworkVariable(netId, "p2p_client_list",
					modifiedClientList.toString());
			mWifiNative.saveConfig();
			return true;
		}

		private void setWifiP2pInfoOnGroupFormation(
		...
		private String getPersistedDeviceName() {
			String deviceName = Settings.Global.getString(
					mContext.getContentResolver(), "wifi_p2p_device_name");
			if (deviceName == null) {
				/*
		...
		private boolean setWfdInfo(WifiP2pWfdInfo wfdInfo) {
			...
			if (!wfdInfo.isWfdEnabled()) {
				...
				success = mWifiNative.setWfdEnable(true)
						&& mWifiNative.setWfdDeviceInfo(wfdInfo
								.getDeviceInfoHex());
			}
		...
		private void initializeP2pSettings() {
			...
			mWifiNative
					.setConfigMethods("virtual_push_button physical_display keypad");
			...
			String countryCode = Settings.Global.getString(
					mContext.getContentResolver(), "wifi_country_code");
		...
		private void handleGroupCreationFailure() {
			...
			// Remove only the peer we failed to connect to so that other
			// devices discovered
			...
			mServiceDiscReqId = null;
			sendMessage(FlockWifiP2pManager.DISCOVER_PEERS);
		}
		...
		private void handleGroupRemoved() {
			...
			NetworkUtils.resetConnections(mGroup.getInterface(),
			...
			// Remove only peers part of the group, so that other devices
			// discovered
			// that have not timed out still remain in list for connection
			...
			if (mTempoarilyDisconnectedWifi) {
				mWifiChannel.sendMessage(
						FlockWifiP2pService.DISCONNECT_WIFI_REQUEST, 0);
		...
		/*
		 * arg2 on the source message has a hash code that needs to be retained
		 * in replies see WifiP2pManager for details
		 */
		...
		private boolean updateSupplicantServiceRequest() {
			...
			mServiceDiscReqId = mWifiNative.p2pServDiscReq("00:00:00:00:00:00",
					sb.toString());
		...
		private void sendServiceResponse(WifiP2pServiceResponse resp) {
			for (ClientInfo c : mClientInfoList.values()) {
				WifiP2pServiceRequest req = c.mReqList.get(resp
						.getTransactionId());
				if (req != null) {
					Message msg = Message.obtain();
					msg.what = FlockWifiP2pManager.RESPONSE_SERVICE;
					msg.arg1 = 0;
					msg.arg2 = 0;
					msg.obj = resp;
					try {
		...
		/**
		 * We dont get notifications of clients that have gone away. We detect
		 * this actively when services are added and throw them away.
		 * 
		...
		private void clearClientDeadChannels() {
			...
			for (ClientInfo c : mClientInfoList.values()) {
				Message msg = Message.obtain();
				msg.what = FlockWifiP2pManager.PING;
				msg.arg1 = 0;
				msg.arg2 = 0;
				msg.obj = null;
				try {
		 ...
		 *            if true and the specified channel info does not exist,
	...
	/**
	 * Information about a particular client and we track the service discovery
	 * requests and the local services registered by the client.
	 */
	private class ClientInfo {

		/*
		 * A reference to WifiP2pManager.Channel handler. The response of this
		 * request is notified to WifiP2pManager.Channel handler
		 */
